...
| **Fonctionnalités** | Recherche de ticker, analyse complète, watchlist multi-utilisateurs, positions (achat + vente), P&L réalisé/latent, conseil quotidien immutable (généré automatiquement par le scheduler pour chaque position ouverte), dashboard admin KPIs, alertes email automatisées (changement de recommandation, variation > 5&nbsp;%, TP/SL, **changement du conseil position** TENIR→ALLÉGER…, **gap pré-marché** : conseil réévalué avant l'ouverture si l'écart overnight dépasse max(2&nbsp;%, 1×ATR)). **Backtest 2 ans** (rejeu du scoring jour par jour, taux de réussite à 5/20j, courbes stratégie vs buy & hold), **attribution par signal** (fiabilité de chaque critère en épisodes), **régime de marché QQQ** (score ajusté selon le contexte NASDAQ, pondéré par le R² ticker/marché), **classifieur probabiliste** P(hausse 20j) en régression logistique avec validation walk-forward, **calibration adaptative des poids par ticker** (chaque critère technique modulé par sa fiabilité mesurée sur 2 ans — shrinkage bayésien, bornes [0;1.5], min 6 épisodes, ajustements affichés), **seuils de conseil normalisés par la volatilité** (stop/TP/renforcement en multiples d'ATR bornés autour de la config utilisateur) + **stop suiveur** (ALLÉGER si repli > 2×ATR depuis le plus haut atteint depuis l'achat) + bouton « ⚡ Seuils SDE » qui pré-remplit les objectifs de prix TP/SL depuis ces mêmes seuils (l'utilisateur valide). Suite à l'audit de la semaine du 13-19.07 sur TMC (score oscillant 6× autour du seuil, signal peu fiable non calibré, achat/vente en aller-retour perdant) : **hystérésis sur la recommandation** (confirmation sur 2 calculs frais ou franchissement net avant de changer d'avis, reco brute toujours visible), **cooldown post-achat** (1 jour, pas d'allégement discrétionnaire sur une position tout juste acquise — le stop loss reste actif), **confiance affichée par conseil** (croise les signaux dominants du jour avec leur fiabilité mesurée sur ce titre précis), **filtre de tendance de fond MA200/52 semaines** (une sortie complète sur signal technique est downgradée en allégement partiel si le titre est structurellement haussier et que le régime de marché ne confirme pas). Puis (22.07) : **RENFORCER sur signal fiable malgré une reco globale VENDRE** (un signal individuel mesuré ≥55% de fiabilité peut outrepasser une reco dominée par des signaux moins fiables, plafonné par la trésorerie), **bannière de divergence technique vs structurel** (alerte visuelle pleine largeur quand fondamental+média et technique pointent dans des directions opposées — cas TMC : news positives noyées 2 semaines dans un score agrégé baissier avant que le cours ne les rattrape en une séance), **fenêtre d'alerte élargie à 06h-23h Paris** (captait mal le pré-marché US, un mouvement nocturne n'était détecté qu'au réveil à 8h). |
